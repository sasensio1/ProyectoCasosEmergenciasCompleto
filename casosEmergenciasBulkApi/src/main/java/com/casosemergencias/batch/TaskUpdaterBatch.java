...
@Resource
public class TaskUpdaterBatch {
	final static Logger LOGGER = Logger.getLogger(TaskUpdaterBatch.class);
	
	@Autowired
	SalesforceRestApiInvokerBatch restApiInvokerBatch;
	
	public void updateTaskInfo() {
		LOGGER.trace("Comienzo del proceso de actualizacion de Task de SalesForce a la base de datos de Heroku");
		...
		restApiInvokerBatch.setObjectName(ConstantesBulkApi.ENTITY_TASK);
		restApiInvokerBatch.updateObjectsWithRestApiInfo();
		LOGGER.trace("Proceso de actualizacion  de Task de SalesForce a la base de datos de Heroku completado");
	}
}