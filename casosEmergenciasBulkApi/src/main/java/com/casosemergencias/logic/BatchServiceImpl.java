...
public class BatchServiceImpl implements BatchService {
	...
	public boolean updateHerokuObjectsFromBulkApi(BulkApiInfoContainerBatch bulkApiInfoContainer, String processId) {
		...
		historicoProcess.setObject(bulkApiInfoContainer.getEntityName());
			...
			if (bulkApiInfoContainer != null && bulkApiInfoContainer.getRecordsMap() != null) {
				if (bulkApiInfoContainer.getTotalRecords() > 0) {
					...
					cont = 0;
					String processInsertErrorCause = "";
					...
					historicoProcessInsert.setErrorCause(insertOk ? null : processInsertErrorCause);
					...
					cont = 0;
					String processUpdateErrorCause = "";
					...
					historicoProcessUpdate.setSuccess(insertOk);
					historicoProcessUpdate.setErrorCause(insertOk ? null : processUpdateErrorCause);
					historicoProcessUpdate.setProcessedRecords(cont);
					historicBatchDAO.insertHistoric(historicoProcessUpdate);
					
					//Registros a eliminar
					cont = 0;
					String processDeleteErrorCause = "";
					...
					historicoProcessDelete.setObject(bulkApiInfoContainer.getEntityName());
					...
					historicoProcessDelete.setSuccess(insertOk);
					historicoProcessDelete.setErrorCause(insertOk ? null : processDeleteErrorCause);
		...
		historicoProcess.setErrorCause(resultOk ? null : processErrorCause);
