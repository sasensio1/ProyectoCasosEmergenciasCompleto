...
@Resource
public class UserUpdaterBatch {
	final static Logger LOGGER = Logger.getLogger(UserUpdaterBatch.class);
	
	@Autowired
	SalesforceRestApiInvokerBatch restApiInvokerBatch;
	
	public void updateUserInfo() {
		LOGGER.trace("Comienzo del proceso de actualizacion de User de SalesForce a la base de datos de Heroku");
		...
		restApiInvokerBatch.setObjectName(ConstantesBulkApi.ENTITY_USER);
		restApiInvokerBatch.updateObjectsWithRestApiInfo();
		LOGGER.trace("Proceso de actualizacion  de User de SalesForce a la base de datos de Heroku completado");
	}
}